...
function Column(props) {
    const { textNumber, bgColor, mode, data, dataLength, searchValue,searchType } = props

    // Making Fake Data
    const cards = []

    data.forEach(item => {
        cards.push(<Card key={item.userTask_id} mode={mode} data={item} searchValue={searchValue} searchType={searchType}/>);
    });
